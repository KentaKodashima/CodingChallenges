...
var subdomainVisits = function(cpdomains) {
  ...
  for (let i = 0; i < cpdomains.length; i++) {
    // Split the string into a number and domain
    let valAndKey = cpdomains[i].split(' ')
    // Put the original domain wiht the number into the obj
    counter[valAndKey[1]] ? counter[valAndKey[1]] += parseInt(valAndKey[0]) : counter[valAndKey[1]] = parseInt(valAndKey[0])
    // Split the domain into parts
    let domainParts = valAndKey[1].split('.')

    if (domainParts.length === 3) {
      let mainDomain = `${domainParts[1]}.${domainParts[2]}`
      counter[mainDomain] ? counter[mainDomain] += parseInt(valAndKey[0]) : counter[mainDomain] = parseInt(valAndKey[0])
      let extension = domainParts[2]
      counter[extension] ? counter[extension] += parseInt(valAndKey[0]) : counter[extension] = parseInt(valAndKey[0])
    } else {
      let extension = domainParts[1]
      counter[extension] ? counter[extension] += parseInt(valAndKey[0]) : counter[extension] = parseInt(valAndKey[0])
    }
  }
...
// console.log(subdomainVisits(["900 google.mail.com", "50 yahoo.com", "1 intel.mail.com", "5 wiki.org"]).toString())
const arr = subdomainVisits([
  "2777 nak.mkw.co",
  "654 yaw.lmm.ca",
  "3528 jyx.arz.us",
